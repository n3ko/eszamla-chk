	...
	const cols = [
		{
			title: "OK",
			dataIndex: "ok",
			render: ok => <span className={styles[ok ? "✔️": "❌"]}></span>
		},
		{
			title: "Név",
			dataIndex: "name",
				...
				reader.onload = async r => {
					...
					if (id == fileList.length-1) {
						...
						allHash.map((hash,id) => {
							...
							fileList[id].ok = bytesToHexString(hash).toUpperCase() == (chk as String).toUpperCase()
						})
						setState({ fileList });			
					}
	...
	return (
		<Layout>
			...
			<Content className={styles.content}>
				...
				<Table dataSource={state.fileList} columns={cols} pagination={false} />
			</Content>
		</Layout>
	)
}
